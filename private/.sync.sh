...
#-------------------------------------------------------------------
#~ @(#) Name : .sync.sh
#~ @(#) Desc : Sync projet and deploy in my host 
#~ @(#) version : na
...
/usr/bin/ssh -p $PORT_TARGET $USER_TARGET@$HOST_TARGET "mkdir -p ${PATH_TARGET}/sh"
...
/usr/bin/ssh -p $PORT_TARGET $USER_TARGET@$HOST_TARGET "mkdir -p ${NODE_MODULE_TARGET}/sh"
/usr/bin/ssh -p $PORT_TARGET $USER_TARGET@$HOST_TARGET "mkdir -p ${NODE_MODULE_TARGET}/docs"
