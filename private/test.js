...
//		.setCharacteristic(Characteristic.SerialNumber, "042-SN-20190407-" + packageFile.version)
	...
	this.macOSXService = new Service.FanSensor(that.name);
	var currentFanCharacteristic = this.macOSXService.getCharacteristic(Characteristic.CurrentFan);
	...
	//	.on('get', this.getFan.bind(this));
		...
		function getCurrentFan() {
			...
			fan = fanVal;
			that.log.debug("update currentFanCharacteristic value: " + fanVal);
			return fanVal;
	}
	...
	//ADM 
	currentFanCharacteristic.updateValue(getCurrentFan());
	if(that.updateInterval) {
		setInterval(() => {
			currentFanCharacteristic.updateValue(getCurrentFan());
			
			that.log("Fan: " + fan);
			this.fakeGatoHistoryService2.addEntry({time: new Date().getTime() / 1000, fan: fan});
			//this.fakeGatoHistoryService.addEntry({time: new Date().getTime(), temp: temp});
	...
	currentFanCharacteristic.on('get', (callback) => {
		callback(null, getCurrentFan());
	});
}
